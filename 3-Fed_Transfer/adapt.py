    ...
    for model_id in tqdm(range(len(train_data_sets))):
        ...
        for internal_epoch in range(1, helper.adaptation_epoch + 1):
            ...
            for batch_id, batch in enumerate(data_iterator):
                ...
                #writer.add_scalar('loss_train',running_loss/1000,internal_epoch)
                loss.backward()
                if helper.data_type == 'text':
        ...
        if (model_id+1)%2==0 or (model_id+1)==len(train_data_sets):
...
if __name__ == '__main__':
    ...
    if not adaptation_helper.only_eval:
        ...
        #for federated_round in tqdm(range(adaptation_helper.start_round, adaptation_helper.total_rounds + 1)):
            #logger.info(f'training in round: {federated_round}')
            #start_time = time.time()

            #subset_data_chunks = random.sample(participant_ids[1:], adaptation_helper.no_models)
            #logger.info(f'Selected models: {subset_data_chunks}')
            #t = time.time()
            #train_sets = [(pos, adaptation_helper.train_data[pos]) for pos in subset_data_chunks]
        t1 = time.time()
 ...
 #accumulate weight of all participants.
    """weight_accumulator = dict()

    for name, data in target_model.state_dict().items():
        #don't scale tied and modified weights:
        if adaptation_helper.tied and name == 'decoder.weight' or '__'in name:
            continue
        if helper.aggregation_type == 'averaging':
            ...
            weight_accumulator[name] = torch.zeros([adaptation_helper.no_models] + weight_per_model)
